...
def make_month_data(data,i):
    ...
    for x in np.arange(0,1.1,0.1):
        x = round(x,1)
        res['{i}-q{x}'.format(i=i,x=x)] = datayuefen.quantile(x, axis=1)
    return res
def make_y_data(data,i):
    ...
    return res
def make_fin_month_data(data,data_1,mon):
    data_1_lie = []
...
data_1 = pd.read_csv(r"xgbdata3.csv",index_col=0,header=0)
users = data_1.index
data=data[data.index.isin(users)]
train_data = pd.DataFrame()
verify_data = pd.DataFrame()
for mon in range(1,10):
    df_month1 = make_fin_month_data(data, data_1, mon)
    df_month2 = make_fin_month_data(data, data_1, mon + 1)
    df_month3 = make_fin_month_data(data, data_1, mon + 2)
    df1 = pd.merge(df_month1, df_month2, how='left', left_index=True, right_index=True)
    df1 = pd.merge(df1, df_month3, how='left', left_index=True, right_index=True)
    df1 = df1.loc[:,sorted(df1.columns)]
    df2 = make_y_data(data,mon+5)
    df1 = pd.merge(df1, df2, how='left', left_index=True, right_index=True)
    del df_month1
    del df_month2
    del df_month3
